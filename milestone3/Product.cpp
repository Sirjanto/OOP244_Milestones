...
namespace AMA {
	...
	std::fstream& Product::store(std::fstream& file, bool newLine) const {
		file << productSku << "," << productName << "," << productUnit << "," << taxStatus << ","
	...
	std::fstream& Product::load(std::fstream& file) {
		...
		if (file.is_open()) {
			file.getline(tSku, max_sku_length, ',');
			file.getline(tName, max_name_length, ',');
			file.getline(tUnit, max_unit_length, ',');
			file >> tTaxStatus;
			file.ignore();
			file >> tPrice;
			file.ignore();
	...
	bool Product::operator>(const Product& rhs) const {
		if (strcmp(productName, rhs.productName) > 0) {
			return true;
		}
		else {
			return false;
	...
	double operator+=(double& added, const Product& obj) {
